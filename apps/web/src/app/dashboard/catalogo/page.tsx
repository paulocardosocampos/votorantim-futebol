        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            <nav className="bg-votorantim-blue p-4 text-white shadow-lg sticky top-0 z-50">
                <div className="max-w-7xl mx-auto flex justify-between items-center px-4">
                    <div className="flex items-center">
                        ...
                        <h1 className="text-xl font-bold uppercase tracking-tight italic">Catálogo de <span className="text-votorantim-green font-black">Prêmios</span></h1>
                    </div>
                    <div className="flex items-center gap-4">
                        ...
                        {user && (
                            <div className="bg-white/10 px-4 py-1 rounded-full text-sm font-bold flex items-center border border-white/10">
                                <span className="text-votorantim-green mr-2 animate-pulse">●</span>
            ...
            <main className="max-w-7xl mx-auto p-4 sm:p-8">
                <header className="mb-12 text-center sm:text-left flex flex-col sm:flex-row items-end justify-between gap-4">
                    <div>
                        <h2 className="text-5xl font-black text-votorantim-blue uppercase tracking-tighter leading-none">Escolha seu <span className="text-votorantim-green">Destino</span></h2>
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center p-20 gap-4">
                        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-votorantim-blue"></div>
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-votorantim-blue/40">Carregando Experiências...</p>
                    </div>
                ) : packages.length === 0 ? (
                    <div className="bg-white rounded-[3rem] p-20 text-center shadow-2xl border border-gray-100 italic">
                        ...
                        <button className="mt-4 text-votorantim-blue font-black uppercase text-xs tracking-widest hover:underline" onClick={() => router.push('/dashboard/enviar-nota')}>
                    ...
                    <div className="space-y-20">
                        ...
                        <section>
                            <div className="flex items-center gap-4 mb-8">
                                <div className="h-px bg-votorantim-blue/10 flex-grow"></div>
                                <h3 className="text-xs font-black uppercase tracking-[0.4em] text-votorantim-blue/40">Destaques por Campeonato</h3>
                                <div className="h-px bg-votorantim-blue/10 flex-grow"></div>
                            </div>

                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {championships.map(name => (
                                    <div key={name} className="bg-votorantim-blue rounded-3xl p-6 relative overflow-hidden group cursor-pointer hover:scale-[1.02] transition-all">
                                        ...
                                        <p className="text-[10px] font-black uppercase tracking-widest text-votorantim-green mb-2">Campeonato</p>
                        ...
                        {championships.map(champ => (
                            <section key={champ}>
                                <div className="flex items-center justify-between mb-8 border-b-2 border-votorantim-blue/5 pb-4">
                                    <h3 className="text-3xl font-black text-votorantim-blue uppercase tracking-tighter">{champ}</h3>
                                    <span className="bg-votorantim-blue/5 px-4 py-1 rounded-full text-[10px] font-black text-votorantim-blue uppercase tracking-widest">
                                        Ver Todos do {champ}
                                    </span>
                                </div>
                                        ...
                                        <div key={pkg.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-xl hover:shadow-2xl transition-all group border border-gray-100 flex flex-col group/card">
                                            <div className="h-60 bg-gray-200 relative overflow-hidden">
                                                {pkg.imageUrl ? (
                                                    ...
                                                    <div className="w-full h-full bg-votorantim-blue flex items-center justify-center text-white/10 group-hover/card:scale-110 transition-transform">
                                                ...
                                                <div className="absolute top-6 left-6">
                                                    <div className="bg-votorantim-green text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-xl flex items-center gap-2">
                                                ...
                                                <div className="absolute bottom-6 right-6 bg-white/95 backdrop-blur-md px-5 py-2.5 rounded-[1.5rem] shadow-2xl border border-white">
                                                    ...
                                                    <p className="text-2xl font-black text-votorantim-blue leading-none mt-1.5">{pkg.priceCoins} <span className="text-[10px] text-votorantim-green uppercase">Coins</span></p>
                                                </div>
                                            </div>

                                            <div className="p-8 flex-grow">
                                                <h3 className="text-2xl font-black text-votorantim-blue mb-2 uppercase leading-[0.9] group-hover/card:text-votorantim-green transition-colors">{pkg.teamMatch}</h3>
                                                <div className="flex items-center text-gray-400 text-xs font-bold uppercase tracking-widest mt-4">
                                                    <svg className="w-4 h-4 mr-2 text-votorantim-green" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                                    {pkg.location}
                                                </div>
                                                <div className="flex items-center text-gray-400 text-xs font-bold uppercase tracking-widest mt-2">
                                                    <svg className="w-4 h-4 mr-2 text-votorantim-green" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                                ...
                                                <button
                                                    ...
                                                    className="mt-8 w-full bg-votorantim-blue text-white py-5 rounded-[1.5rem] font-black uppercase tracking-widest text-xs shadow-xl active:scale-95 hover:bg-votorantim-blue/90 transition-all disabled:bg-gray-100 disabled:text-gray-300 disabled:shadow-none translate-z-0 group-hover/card:bg-votorantim-green group-hover/card:text-votorantim-blue"
                                                >
                                                    {user?.coinBalance < pkg.priceCoins ? 'Coins Insuficientes' : 'Resgatar Agora'}
                                                </button>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <section className="bg-votorantim-blue/5 rounded-[4rem] p-12">
                            <div className="text-center mb-12">
                                <h3 className="text-4xl font-black text-votorantim-blue uppercase leading-none tracking-tighter italic">Nossa <span className="text-votorantim-green">Variedade</span></h3>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                {packages.map(pkg => (
                                    <div key={pkg.id} className="bg-white p-4 rounded-2xl shadow-md border border-gray-100 flex flex-col items-center text-center hover:shadow-xl transition-all cursor-pointer" onClick={() => router.push(`/dashboard/redimir/${pkg.id}`)}>
                                        <div className="w-12 h-12 bg-votorantim-blue/5 rounded-full flex items-center justify-center mb-3">
                                            <svg className="w-6 h-6 text-votorantim-blue/30" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" /></svg>
                                        </div>
                                        <p className="text-[8px] font-black text-votorantim-green uppercase mb-1">{pkg.championship}</p>
                                        <h5 className="text-[10px] font-black text-votorantim-blue uppercase leading-tight line-clamp-1">{pkg.teamMatch}</h5>
                                    </div>
                                ))}
                            </div>
                        </section>
