        ...
        <div className="min-h-screen bg-gray-50 flex">
            ...
            <main className="flex-1 p-4 sm:p-12">
                <header className="mb-10">
                    ...
                    <h2 className="text-4xl font-black text-votorantim-blue uppercase">Novo Usuário</h2>
                ...
                <form onSubmit={handleSubmit} className="max-w-2xl bg-white p-10 rounded-[2rem] shadow-2xl border border-gray-100 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input name="name" required value={form.name} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-medium focus:border-votorantim-blue transition-all" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">E-mail</label>
                            <input name="email" type="email" required value={form.email} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-medium focus:border-votorantim-blue transition-all" />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">CPF ou CNPJ</label>
                                <input name="document" required value={form.document} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-medium focus:border-votorantim-blue transition-all" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Senha Provisória</label>
                                <input name="passwordHash" type="password" required value={form.passwordHash} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-medium focus:border-votorantim-blue transition-all" />
                            </div>
                        </div>
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-votorantim-green text-votorantim-blue py-5 rounded-2xl font-black text-sm shadow-xl shadow-votorantim-green/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 uppercase tracking-widest mt-8"
                    >
