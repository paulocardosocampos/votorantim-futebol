...
export default function EditarUsuarioPage() {
    ...
    if (loading) return <div className="p-20 text-center animate-pulse font-black text-votorantim-blue uppercase tracking-widest text-xs">Carregando perfil...</div>;
        ...
        <div className="min-h-screen bg-gray-50 flex font-sans">
            ...
            <main className="flex-1 p-4 sm:p-12">
                <header className="mb-10">
                    <button onClick={() => router.back()} className="text-votorantim-blue mb-4 flex items-center text-xs font-black uppercase tracking-widest hover:opacity-70 transition-opacity">
                    ...
                    <h2 className="text-4xl font-black text-votorantim-blue uppercase">Editar Usuário</h2>
                    <p className="text-gray-500 mt-2">Ajuste dados cadastrais, saldo de coins ou nível de acesso.</p>
                </header>
                ...
                <form onSubmit={handleSubmit} className="max-w-2xl bg-white p-10 rounded-[2rem] shadow-2xl border border-gray-100 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <select name="role" value={form.role} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-bold focus:border-votorantim-blue transition-all">
                        ...
                        <div>
                            ...
                            <input name="name" value={form.name} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-medium focus:border-votorantim-blue transition-all" />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">E-mail</label>
                                <input name="email" value={form.email} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-medium focus:border-votorantim-blue transition-all" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Saldo de Coins</label>
                                <input name="coinBalance" type="number" value={form.coinBalance} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-black focus:border-votorantim-blue transition-all" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Documento (CPF/CNPJ)</label>
                            <input name="document" value={form.document} onChange={handleChange} className="w-full bg-gray-50 border-2 border-gray-50 rounded-xl px-4 py-4 text-votorantim-blue font-medium focus:border-votorantim-blue transition-all" />
                        </div>
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={saving}
                        className="w-full bg-votorantim-blue text-white py-5 rounded-2xl font-black text-sm shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 uppercase tracking-widest mt-8"
                    >
